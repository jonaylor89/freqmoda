...
use crate::{
    ...
    models::{ChatRequest, ChatResponse, OpenAIMessage},
    services::{openai::OpenAIService, streaming_engine::StreamingEngineService},
    state::AppState,
};
...
pub async fn chat(
    ...
    // Debug: Log the actual conversation history being sent to OpenAI
    ...
    let openai = OpenAIService::new(
        ...
        state.settings.openai.api_key.clone(),
        state.settings.openai.base_url.clone(),
        state.settings.openai.model.clone(),
    );

    // Send to OpenAI
    tracing::debug!("Sending message to OpenAI API");
    let openai_response = match openai.send_message(openai_messages, None).await {
        Ok(response) => {
            tracing::debug!(
                "Received response from OpenAI with {} choices",
    ...
    let choice = openai_response
        ...
        .into_iter()
        .next()
        .ok_or_else(|| AppError::OpenAI("No choices in response".to_string()))?;
    ...
    if let Some(tool_calls) = choice.message.tool_calls {
        for tc in &tool_calls {
            ...
            match tc.function.name.as_str() {
                ...
                _ => {
                    tracing::warn!("Unknown tool: {}", tc.function.name);
                }
            }
        }
    }
