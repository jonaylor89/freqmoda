...
async fn e2e_multi_user_concurrent_conversations() {
    ...
    app.mocks
        .openai_mock
        .mock_chat_completion_with_text_response("Response from OpenAI")
        .await;
...
async fn e2e_conversation_with_audio_processing_integration() {
    ...
    app.mocks.openai_mock.mock_chat_completion_success().await;
    app.mocks
        .streaming_mock
    ...
    // Step 3: Simulate direct audio processing call (as if triggered by OpenAI)
...
async fn e2e_data_consistency_across_failures() {
    ...
    // Simulate partial failure - message sent but OpenAI fails
    app.mocks
        .openai_mock
    ...
    // Check if user message was still recorded despite OpenAI failure
    let count_after_failure = app.get_message_count(conv_uuid).await;
    assert!(
        count_after_failure > initial_count,
        "User message should be recorded even if OpenAI fails"
    );
...
async fn e2e_graceful_degradation_scenarios() {
    ...
    // Scenario 1: OpenAI API degraded but not completely down
    app.mocks.openai_mock.mock_rate_limit_error().await;
