...
use crate::models::{AudioSample, AudioVersion, Conversation, Message, User};
use sqlx::PgPool;
...
// Audio versions functions
pub async fn create_audio_version(
    pool: &PgPool,
    sample_id: &str,
    session_id: &str,
    ...
    let result = sqlx::query_as!(
        AudioVersion,
        r#"
        INSERT INTO audio_versions (sample_id, session_id, conversation_id, audio_url, description)
        VALUES ($1, $2, $3, $4, $5)
        RETURNING *
        "#,
        sample_id,
...
pub async fn get_audio_versions_for_session(
    pool: &PgPool,
    sample_id: &str,
    session_id: &str,
) -> Result<Vec<AudioVersion>> {
    let result = sqlx::query_as!(
        AudioVersion,
        r#"
...
pub async fn get_latest_audio_version(
    pool: &PgPool,
    sample_id: &str,
    session_id: &str,
) -> Result<Option<AudioVersion>> {
    let result = sqlx::query_as!(
        AudioVersion,
        r#"
        SELECT * FROM audio_versions
