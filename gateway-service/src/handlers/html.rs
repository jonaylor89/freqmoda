...
use crate::{
    database::{
        create_audio_version, get_all_audio_samples, get_audio_sample_by_key,
        get_audio_versions_for_session,
    },
    error::AppError,
    handlers::chat::chat,
    models::{AudioSample, AudioVersion, ChatRequest},
    state::AppState,
};
...
struct SampleChatTemplate {
    ...
    audio_versions: Vec<AudioVersion>,
    posthog_api_key: Option<String>,
    posthog_host: Option<String>,
}
...
pub async fn index_page(State(state): State<AppState>) -> Result<Html<String>, StatusCode> {
    ...
    let posthog_config = state.settings.posthog.as_ref();
    let template = IndexTemplate {
        samples,
        posthog_api_key: posthog_config.map(|c| c.api_key.expose_secret().to_string()),
        posthog_host: posthog_config.map(|c| c.host.to_string()),
    };
...
pub async fn sample_chat_page(
    ...
    // Get audio versions for this sample and session
    let audio_versions = get_audio_versions_for_session(&state.db, &sample_id, &session_id)
        .await
        .unwrap_or_else(|e| {
            error!("Failed to get audio versions: {:?}", e);
            Vec::new()
        });
...
pub async fn sample_chat_form_handler(
    ...
    match chat(State(state.clone()), Json(chat_request)).await {
        Ok(response) => {
            ...
            // Check if the response contains a processed audio URL and create a version
            if let Some(audio_url) = extract_audio_url(
                &chat_response.message,
                &state.settings.streaming_engine.base_url,
            ) {
                let description = format!("Processed: {}", form.message);
                if let Err(e) = create_audio_version(
                    &state.db,
                    &sample_id,
                    &session_id,
                ...
                .await
                {
                    error!("Failed to create audio version: {:?}", e);
        ...
        Err(e) => {
            ...
            match &e {
                ...
                AppError::Claude(claude_err) => {
                    error!("Claude API error details: {}", claude_err);
                }
...
async fn store_sample_message(
    ...
    // TODO: Extract thinking and tool calls from content for enhanced interaction
    let message = DisplayMessage {
        role: role.to_string(),
        content: cleaned_content,
        audio_url: audio_url.unwrap_or_default(),
        thinking: None,         // TODO: Extract from Claude response
        tool_calls: Vec::new(), // TODO: Extract from Claude response
    };
