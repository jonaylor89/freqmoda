...
echo "🏗️  Setting up Gateway Service Infrastructure (External Services)"
...
SERVICE_ACCOUNT_NAME="gateway-service"
SERVICE_ACCOUNT="$SERVICE_ACCOUNT_NAME@$PROJECT_ID.iam.gserviceaccount.com"

if ! gcloud iam service-accounts describe $SERVICE_ACCOUNT --project=$PROJECT_ID >/dev/null 2>&1; then
    gcloud iam service-accounts create $SERVICE_ACCOUNT_NAME \
        --display-name="Gateway Service Account" \
        --description="Service account for Gateway Service Cloud Run deployment" \
        --project=$PROJECT_ID
...
echo "./scripts/deploy-gateway-service.sh $PROJECT_ID $REGION"
